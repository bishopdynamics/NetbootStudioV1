...
if [ -z "$1" ]; then
	...
	echo "deploying last build in [$BUILD_DIR]"
else
	COMMIT_ID="${1}"
	BUILD_DIR="${CURRENT_DIR}/dist-versions/${COMMIT_ID}"
	echo "deploying a previous build from commit $COMMIT_ID in [$BUILD_DIR]"
fi

# sanity check the build
if [ ! -f "${BUILD_DIR}/checksums.txt" ]; then
	echo "something wrong with build dir [${BUILD_DIR}], could not find checksums.txt"
	exit 1
fi
