...
STAGE4_FOLDER = '%s/stage4' % LOCAL_STORAGE_FOLDER
...
class NSHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if clean_url == '/stage2.ipxe':
            ...
            self.get_loginpage()
        elif clean_url in ('/stage4.sh', '/stage4.bat'):
            self.get_stage4(self.path)
        else:
    ...
    def get_stage4(self, fullurl):
        # endpoints are /stage4.sh /stage4.bat
        # windows is a little picky about file names, so we support multiple endpoints with the same method
        # actual name of the file within stage4/ is totally irrelevant to the client, the client receives file named matching the parameter
        urlparams = dict(parse.parse_qsl(parse.urlsplit(fullurl).query))
        client_ipaddress = self.client_address[0]
        requested_file = urlparams['file']
        file_path = "%s/%s" % (STAGE4_FOLDER, requested_file)
        try:
            with open(file_path, 'r') as _fh:
                content = _fh.read()
            logmessage('Responding to %s, with stage4 file: %s' % (client_ipaddress, file_path))
            self.send_response(200)
            self.send_header('Content-type', 'text/plain')
            self.end_headers()
            self.wfile.write(bytes(content, 'UTF-8'))
...
def LoadClientList():
    ...
    try:
        ...
        logmessage('successfully loaded client list from file')
    except FileNotFoundError:
...
def SaveClientList():
    ...
    try:
        with open(CLIENT_LIST_FILE, 'w', encoding='utf-8') as f:
            json.dump(CLIENT_LIST, f, indent=2)
        logmessage('Saved client list to file: %s' % CLIENT_LIST_FILE)
    except Exception as e:
